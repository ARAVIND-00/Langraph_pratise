...
def human_node(state:State):
    ...
    if user_feedback.strip().lower()=="done":
        return Command( 
            update={"human_feedback":[HumanMessage(content=str(user_feedback))]

            },goto="end_node"
        

        )
    return Command(update={"human_feedback":[HumanMessage(content=str(user_feedback))]},goto="model")
...
def end_node(state:State):
    ...
    #return {"linkedin_topic":state["linkedin_topic"],"generated_post": state["generated_post"], "human_feedback": state["human_feedback"]}
    return {}
...
initial_state={ "linkedin_topic":linkedin_topic
    }
for chunk in app.stream(initial_state,config=thread_config):
    ...
    for node_id, value in chunk.items():
        if(node_id == "__interrupt__"):
            while True: 
                user_feedbackA = input("Provide feedback (or type 'done' when finished): ")

                # Resume the graph execution with the user's feedback
                app.invoke(Command(resume=user_feedbackA), config=thread_config)

                # Exit loop if user says done
                if user_feedbackA.lower() == "done":
                    break
